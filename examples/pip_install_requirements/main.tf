...
resource "terraform_data" "requirements_txt_md5sum" {
  input = filemd5(var.requirements_txt_location)
}

resource "null_resource" "layer_folder_generation" {
  lifecycle {
    replace_triggered_by = [terraform_data.requirements_txt_md5sum]
  }
  provisioner "local-exec" {
    command = "./layer_folder_from_requirements.sh ${var.requirements_txt_location} ./lambda_layer"
  }
}
